...
RSpec.describe ReleaseCatalog do
  describe ".current_version" do
    it "uses the newest release entry as the app version" do
      expect(described_class.current_version).to eq("2.2.0")
    end
  end
  ...
  describe ".unread_for" do
    ...
    it "alerts users on 2.1.0 about the latest release" do
      user = build(:user, last_seen_release_version: "2.1.0")

      expect(described_class.latest_unread_for(user)&.version).to eq("2.2.0")
      expect(described_class.latest_unread_for(user)&.title).to eq("Broader account imports and clearer balances")
    end
  end
end
